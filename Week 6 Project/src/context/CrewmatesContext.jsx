...
export function CrewmatesProvider({ children }) {
  ...
  const loadCrewmates = useCallback(async () => {
    try {
      ...
      setError(err instanceof Error ? err.message : 'Unable to load your crew.')
    } finally {
      setLoading(false)
    }
  }, [])
  ...
  const addCrewmate = useCallback(async (payload) => {
    try {
      ...
      const message = err instanceof Error ? err.message : 'Unable to add that crewmate.'
      setError(message)
  ...
  const editCrewmate = useCallback(async (id, updates) => {
    try {
      ...
      const message = err instanceof Error ? err.message : 'Unable to update that crewmate.'
      setError(message)
      throw new Error(message)
    } finally {
      setIsSaving(false)
  ...
  const removeCrewmate = useCallback(async (id) => {
    try {
      ...
      const message = err instanceof Error ? err.message : 'Unable to delete that crewmate.'
      setError(message)
      throw new Error(message)
    } finally {
      setIsSaving(false)
